...
const CustomCircle = (props: any) => {
  ...
  return (
    <svg
      ...
      strokeWidth="2" // Reduced stroke width for smaller size
      strokeLinecap="round"
...
function FinishTest({}: Props) {
  ...
  // useEffect(() => {
  //   const data = eachWordTimeSpent.slice(1).map((time, index) => {
  //     const { totalTimeMs } = calculateTimeDiff(eachWordTimeSpent[index], time);
  //     return {
  //       index: index + 1,
  //       wpm: calculateWpm(1, totalTimeMs),
  //       errorCount: eachWordError[index] || 0,
  //     };
  //   });
  //   setChartData(data);
